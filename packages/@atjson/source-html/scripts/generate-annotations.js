...
async function defineHTMLInterface(page) {
  ...
  attributes.push(["dataset", "any"]);

  writeFileSync(
    join(__dirname, "..", "src", "annotations", "global-attributes.ts"),
...
(async () => {
  ...
  const classNames = JSON.parse(
    readFileSync(join(__dirname, "class-names.json")).toString()
  );
  ...
  definitions.forEach(dfn => {
    if (dfn.attributes.length) {
      writeFileSync(
        join(__dirname, "..", "src", "annotations", `${dfn.type}.ts`),
        `// ⚠️ Generated via script; modifications may be overridden
...
`
  ...
  writeFileSync(
    ...
    `// ⚠️ Generated via script; modifications may be overridden
${definitions
  .map(dfn => `import { default as ${dfn.className} } from "./${dfn.type}";`)
  .join("\n")}
